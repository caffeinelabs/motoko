...
// Any
let _ : [var () -> Any] = do {
  let x =  // use block to force inference mode
    [
      func() : Int { bot() },
      func() : Any { bot() },
    ];
...
let _ : [var { a : Nat; b : Nat; var c : Nat } -> ()] = do {
  let x =
    [
      func(_ : { a : Nat; b : Nat }) {},
...
// Variants
let _ : [var () -> { #a : Int; #b : Nat; #c : Nat }] = do {
  let x =
    [
      func() : { #a : Nat; #b : Nat } { bot() },
...
// Functions
let _ : [var () -> (<A, B <: A>(Nat, B) -> (Int, A))] = do {
  let x =
    [
...
let _ : [var (<A, B <: A>(Int, A) -> (Nat, B)) -> ()] = do {
  let x =
    [
