...
actor A {
  ...
  func foo(next : () -> async ()) : async () {
    ...
    Cycles.add<system> 3000;
    await next()
  };

  func bar(next : () -> async ()) : async () = async {
    Cycles.add<system> 4000;
    await next()
  };
  ...
  public func test() : async () {
    ...
    Cycles.add<system> 101;
    assert 42 == (await closA(39));
    Cycles.add<system> 102;
    assert 3 == (await closB());

    Cycles.add<system> 1000;
    let c : async () =
  ...
  public func test2() : async () {
    debugPrint "test2()";
    Cycles.add<system> 1042;
    await async { assert Cycles.available() == 1042 };
  };

  public func test3() : async () {
    debugPrint "test3()";
    oneshot();
    Cycles.add<system> 3456;
    oneshot();
    (with timeout = 5) oneshot();
    Cycles.add<system> 4567;
    A.oneshot();
  };
