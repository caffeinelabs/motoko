...
func _reports_suggestions(n : Nat) : [List.List<Nat>] {
  Array.tabulate<List.List<Nat>>(
    ...
    func(i) = List.fromArray<Nat>(
      // NO warning (kept: the one necessary instantiation)
...
func _applied_unsound_suggestions(n : Nat) : [List.List<Nat>] {
  Array.tabulate(
    ...
    func(i) = List.fromArray(
      // error (M0098): nothing pins the element type
...
func _impl_inst_ok1() {
  // implicit omitted, <?R> now pins T — compiles
  let _grid = withImpl(
    3,
...
func _impl_inst_ok2() {
  // implicit kept, <?R> dropped instead — compiles
  let _grid = withImpl(
    3,
    Impl1.impl,
...
func _impl_inst_error() {
  // both dropped — error (M0098), nothing pins T
  let _grid = withImpl(
    3,
    func _ = Prim.Array_tabulateVar(5, func _ = null),
  );
...
func _impl_impl_ok1() {
  // outer implicit omitted, inner kept — compiles
  let _grid = withImpl(
    3,
...
func _impl_impl_ok2() {
  // outer kept, inner implicit omitted — compiles
  let _grid = withImpl(
    3,
    Impl1.impl,
...
func _inst_inst_ok1() {
  // outer instantiation dropped, inner <?R> kept — compiles
...
func _inst_inst_ok2() {
  // outer kept, inner <?R> dropped — compiles
  let _grid = Prim.Array_tabulate<[var ?R]>(
    3,
    func _ = Prim.Array_tabulateVar(5, func _ = null),
  );
...
func _inst_impl_ok1() {
  // outer instantiation dropped, implicit kept — compiles
  let _grid = Prim.Array_tabulate(
    3,
    func _ = withImpl(5, Impl2.impl, func _ = null),
  );
...
func _inst_impl_ok2() {
  // outer kept, implicit omitted — compiles
  let _grid = Prim.Array_tabulate<[var ?R]>(
    3,
    func _ = withImpl(5, func _ = null),
  );
...
func _all_three() {
  let _grid = withImpl<[var ?R]>(
    // NO warning (kept: pins T for the other two)
    3,
