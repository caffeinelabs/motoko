...
3. Fill in the two optional version fields (see below):
   - **Base version** — overrides the version taken from `Changelog.md`, e.g. `2.0.0`.
     Leave empty to use the changelog version (the usual case when testing a draft).
   - **Version suffix** — the prerelease label, e.g. `beta.0`, `alpha-1`, `rc.1`.
     Leave empty for an official release.
...
6. To make a pre-release:
   ...
   2. Make sure the tag and the name of the release are the same as the generated version, e.g. `2.0.0-beta.0`. Note that there is no need to manually push the tag, it should be created automatically when publishing the pre-release.
   3. Scroll down to the bottom and publish the pre-release.
7. Remember to delete the draft release after testing!

If the draft was published as a pre-release, no further action is needed: the release
artifacts already carry the right names and the compiler inside them reports the same
version via `moc --version`. Publishing a draft creates its tag, which re-triggers this
workflow as a tag `push`; that second run detects the release is already published and
does nothing (see "Version" below and `.github/workflows/release.yml`).

### Version

The version is made of a base version and an optional suffix:

```
<base>[-<suffix>]
```

- The **base** is the *Base version* input when set, otherwise the first matching
  entry in `Changelog.md` **on the selected branch**
  (source: [nix/releaseVersion.nix](nix/releaseVersion.nix)).
- The **suffix** is the *Version suffix* input. Its presence is what makes the run a
  prerelease rather than an official release.

So base `2.0.0` with suffix `beta.0` yields `2.0.0-beta.0`, while an empty base with
suffix `alpha-1` yields `<changelog version>-alpha-1`, e.g. `1.16.1-alpha-1`.

Both inputs are validated by [.github/actions/release-version/](.github/actions/release-version/):
the base must be three dot-separated numeric components, and the suffix may only
contain letters, digits, dots and hyphens.

The generated version is used as the release name and tag, and as the version reported
by `moc --version`. Artifacts are uploaded with the correct names according to the
