...
-   `"canister:<name>"` is a symbolic reference to canister alias `<name>`. The preferred way to resolve the alias is `--actor-id-alias <name> <canisterid> <idl-file>`, which maps `<name>` to `<canisterid>` and supplies the IDL file explicitly, bypassing `--actor-idl`. Alternatively, `--actor-alias <name> <canisterid>` maps the name to a principal and resolves the IDL via `--actor-idl`. For example, `import C "canister:counter"` defines `C` to reference the actor otherwise known as `counter`.
