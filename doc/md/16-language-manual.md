...
-   For each `i` in `(0..implicit_arity(F)]`:
    ...
    **Implicit derivation**: When no direct local value or module-field candidate is found, the compiler additionally searches for *derivable* candidates — first among local values, then among module fields, then among library fields (from unimported modules, but only when the `--implicit-package` flag is set).
    ...
    **Structural derivation**: When derivation also fails, the compiler additionally searches for *structural combiners* — first among local values, then module fields, then library fields (gated on `--implicit-package`). The combiner's parameter name determines the structural kind:
    ...
    - `__record` (parameter type `[(Text, E)] -> R`): handles both unary holes (`SomeRecord -> R`) and binary holes (`(SomeRecord, SomeRecord) -> R` where both args are the same record type). For a unary hole it synthesizes `func($r) { combiner([("f", inst($r.f)), ...]) }` with per-field implicits `FieldType -> E`. For a binary hole it synthesizes `func($r1, $r2) { combiner([("f", inst($r1.f, $r2.f)), ...]) }` with per-field implicits `(FieldType, FieldType) -> E`. The arity is determined by the hole type, not the combiner.
    - `__tuple` (parameter type `[E] -> R`): handles both unary holes (`(A, B, ...) -> R` with at least two elements) and binary holes (`((A, B, ...), (A, B, ...)) -> R` where both args are the same tuple type with ≥ 2 elements). For a unary hole it synthesizes `func($t) { combiner([inst0($t.0), inst1($t.1), ...]) }` with per-element implicits `ElemType_i -> E`. For a binary hole it synthesizes `func($t1, $t2) { combiner([inst0($t1.0, $t2.0), ...]) }` with per-element implicits `(ElemType_i, ElemType_i) -> E`. Tuples with fewer than two elements are not synthesized: single-element tuples reduce to the element type, and unit `()` is treated as a scalar.
