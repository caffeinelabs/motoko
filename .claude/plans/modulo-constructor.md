...
- **Repurpose the worker's `$clos` slot for `parent`.** TRMC-specific. The worker is a Local function with **zero captures** and a **known unique caller** (the wrapper). Today its wasm signature has the standard `(param $clos i64) (param $$parent/0 i64) (param $f i64)`, with the closure slot always passed as `i64.const 0`. Since the worker never enters the function table (no `call_indirect` to it — only the wrapper's direct call and the worker's own `return_call`-by-name), we can break with the uniform Local-function ABI and drop the `$clos` slot entirely, putting `parent` in the env-argument position. Wasm signature shrinks from `(param i64 i64 i64)` to `(param i64 i64)`. Per-iteration win: one fewer stack push at the recursive `return_call` site, one fewer at the wrapper's call site. Implementation hook: special case of the static-closure work in PR #5964 — a Local function with empty environment AND known-unique direct caller can shed the closure parameter rather than just pin its function index.
