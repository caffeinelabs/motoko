...
let rec over_exp (f : exp -> exp) (exp : exp) : exp = match exp.it with
  ...
  | DoVariantE (x, exp1) -> f { exp with it = DoVariantE (x, over_exp f exp1) }
  | BangE (exp1, r) -> f { exp with it = BangE (over_exp f exp1, r) }
  | SlashTagE (exp1, x) -> f { exp with it = SlashTagE (over_exp f exp1, x) }
