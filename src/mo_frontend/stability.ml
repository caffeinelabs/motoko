...
let error_sub s at tf1 tf2 explanation =
  Diag.add_msg s
...
let error_stable_sub s at tf1 tf2 explanation =
  Diag.add_msg s
         ...
         (match Type.compare_field tf1 tf2 with
          | 0 ->
             ...
             begin
               ...
               | Incompatible explanation -> error_sub s at tf1 tf2 explanation
               | Compatible ->
                  match Type.stable_sub_explained context (as_immut tf1.typ) (as_immut tf2.typ) with
                  | Incompatible explanation -> error_stable_sub s at tf1 tf2 explanation
                  | Compatible -> ()
             end;
             go tfs1' tfs2'
          | -1 ->
             ...
             go tfs1' tfs2
          | _ ->
             (if is_required then error_required s at tf2);
...
let match_stab_sig sig1 sig2 : unit Diag.result =
  ...
  let res = Diag.with_message_store (fun s ->
    match_stab_fields s Source.no_region tfs1 tfs2)
  in
