...
%token EOF
(* Never lexed; the dead alternative of the `hd` grammar mode, see ob/bl/hd *)
%token DISALLOWED
...
%type<Mo_def.Syntax.exp -> region -> Mo_def.Syntax.exp> exp_cont(ob) exp_cont(bl) exp_cont_juxta exp_cont_call
%type<Mo_def.Syntax.exp> paren_exp exp_nullary(hd)
%type<Mo_def.Syntax.exp> head(hd) head(bl) if_exp(ob) if_exp(bl) else_branch(ob) else_branch(bl)
...
(* The grammar modes, as conditionals over the two atoms whose opening token can also open something else:
   a record `{ ... }` (or a block/body) and a parenthesized expression `( ... )` (or the v1 `if`/`while` head).
     ob(REC, PAR) — operand position: both are atoms;
     bl(REC, PAR) — block position and heads generally: `{` is a block or body, `(` is still an atom;
     hd(REC, PAR) — the start of a braced `if`/`while` head: neither, so `if (` can only be the legacy parenthesized head.
   DISALLOWED is never lexed, so the `hd` alternative is dead; it only lets the three modes share every rule. *)
%inline ob(REC, PAR) : x=REC { x } | x=PAR { x }
%inline bl(REC, PAR) : x=PAR { x }
%inline hd(REC, PAR) : DISALLOWED { PrimE("dummy") @? at $sloc }
...
(* Two grammar modes say which tokens still belong to the expression and which already belong to the surrounding construct:
   - B ("begin"): what may it START with?
     Under `ob` a leading `{` is a record; under `bl` it is not part of the expression at all — it is a block (statement position)
     or the construct's body (head position); under `hd` a leading `(` is not part of it either — it is the v1 `if`/`while` head.
   - R ("rest"): is this a head? Under `bl` (head mode) the expression may not contain a `{` later on — as a record argument,
     ...
     and no juxtaposed argument (`f x`, `f {a = 1}`) continues it, only a call or index.
   Braced `if`/`while` heads parse as (hd, bl), `switch`/`for` heads as (bl, bl),
   statement position is (bl, ob), and anything parenthesized resets to (ob, ob). *)

exp_arg(B):
  | a=B(record_arg, paren_arg) { a }
  | l=lit
...
(* A postfix continuation is "the rest of a call or indexing": give it the expression parsed so far and it returns that expression with the argument or index attached.
   A juxtaposed argument (`f x`, `f {a = 1}`) only in operand mode: in a head it already belongs to the branch or body that follows. *)
exp_cont(R) :
  | c=R(exp_cont_juxta, exp_cont_call)
    { c }
...
*)
  ...
  | FOR p=pat IN e1=head(bl) e2=block
    { ForE(p, e1, e2, new_loop_flags ()) @? at $sloc }
...
(* MIGRATION BRIDGE — retired in moc v3 (#6352, rule 5: no optional blocks).
   ...
   and the `hd` mode, which exists only because `if (`/`while (` still opens the v1 head. *)
%public exp_nest(R) :
  | e=block
  | e=exp(bl, R)
    { e }
...
(* `if` has two shapes, told apart by the first token of the condition, so no precedence is ever consulted.
   LEGACY(v3): a condition that starts with `(` is exactly the v1 head, the parenthesized expression, and its branches are
   free-form: `if (c) e1 else e2`, `if (c) -1 else 1`. A braced head (`head(hd)`) cannot start with `(`, so after `if (`
   only this shape is live and whatever follows the `)` is the branch.
   Otherwise the condition is a full expression and the branches are blocks — the target syntax of #6352: `if f(x) { } else { }`. *)
if_exp(R) :
  | IF b=paren_exp e1=legacy_body(R) %prec IF_NO_ELSE
    { IfE(b, e1, TupE([]) @? at $sloc) @? at $sloc }
  | IF b=paren_exp e1=legacy_body(R) ELSE e2=legacy_body(R)
    { IfE(b, e1, e2) @? at $sloc }
  | IF b=head(hd) e1=block %prec IF_NO_ELSE
    { IfE(b, e1, TupE([]) @? at $sloc) @? at $sloc }
  | IF b=head(hd) e1=block ELSE e2=else_branch(R)
    { IfE(b, e1, e2) @? at $sloc }

(* the `else` of a braced `if` is a block or another `if` of either shape *)
else_branch(R) :
  | e=block { e }
  | e=if_exp(R) { e }

(* The head of `switch`/`for` (B = bl) and of a braced `if`/`while` (B = hd): any expression short of the statement forms
   (assignments, `return`, nested control flow), which make no sense as a condition — parenthesize in the unlikely case one is needed.
   Under `bl` a `{` is never part of the head, so it always begins the body; under `hd` neither is a leading `(`, see if_exp. *)
head(B) :
  | e=exp_bin(B, bl)
    { e }
  | e1=exp_bin(B, bl) NULLCOALESCE e2=exp(bl, bl)
    { NullCoalesceE(e1, e2) @? at $sloc }
