      ...
      List.map (fun (lab, typ, region)->
          let path =
            { it = dot_module_exp (module_exp in_libs module_ref) ({ it = lab; at = no_region; note = () });
              ...
              note = empty_typ_note; }
          in
          ({ path; typ; module_ref_opt = Some module_ref; id=lab; region } : hole_candidate))
  in
...
and infer_obj env obj_sort exp_opt dec_fields at : T.typ =
  ...
  if not env.pre then begin
    ...
    if (s = T.Actor || s = T.Mixin) && Option.is_some env.enhanced_migration then Static.dec_fields ~allow_system:true env.msgs dec_fields;
