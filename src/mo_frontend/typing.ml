    ...
    if is_in_shared_pat region.left then
      ...
      | Scope.Declaration ->
         warn env region "M0240"
           ~notes:[Format.sprintf "if this is intentional, prefix it with an underscore: `_%s`" id]
      ...
      | Scope.Declaration ->
        warn env region  "M0194"
          ~notes:[Format.sprintf "delete or rename to wildcard `_` or `_%s`" id]
    ...
    | Error (HoleSuggestions mk_suggestions) ->
      ...
      if not env.pre then begin
        let explicit_sug =
          ...
          else [Format.sprintf "Did you mean to explicitly use %s?" (String.concat " or " explicit_suggestions)]
        in
        let import_sug =
          if import_suggestions = [] then
            ...
            Format.sprintf
             ...
             desc desc
          else Format.sprintf "Did you mean to import %s?" (String.concat " or " import_suggestions)
        in
        renaming_hints env;
