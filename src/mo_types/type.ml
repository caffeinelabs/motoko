...
(* Normalises first, unlike the raw discriminators around it: actor-ness is a
   semantic query and its call sites (codegen, IR interpreter) hold notes that
   may not be in head-normal form. *)
let is_actor t = match normalize t with Obj (Actor, _, _) -> true | _ -> false
