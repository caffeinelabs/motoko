...
let argspec =
  ...
  " enable wasm tail-call codegen (`return_call` for tail-positioned calls, both self and cross-function); gives bounded stack and a small cycle reduction; closure-call tail-calls map to `return_call_indirect`";
