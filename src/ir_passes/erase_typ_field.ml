  ...
  *)
  ...
  and t_exp' (exp : exp) =
    ...
    | FuncE (x, s, c, typbinds, args, ret_tys, exp, codecs) ->
      let codecs' = { encoder = Option.map t_exp codecs.encoder;
                      decoder = Option.map t_exp codecs.decoder } in
      FuncE (x, s, c, t_typ_binds typbinds, t_args args, List.map t_typ ret_tys, t_exp exp, codecs')
