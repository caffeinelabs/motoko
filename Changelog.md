...
* motoko (`moc`)
  ...
  * perf: Write-barrier improvements for the incremental GC (e.g. EOP) — multi-value `if (param i64 i64)` keeps `[location, value]` on the wasm stack, and the running-GC check reads a cached `__running_gc` global instead of calling the RTS. −7.7% cycles on barrier-dense `test/bench/iter` (#6111).
